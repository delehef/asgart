...
use palette::{Rgb, Hsv, Gradient, RgbHue, FromColor};
...
impl ChordPlotter {
    ...
    fn ticks(&mut self, step: i64, size: f64, font_size: i32, color: &str) {
        for l in (0..self.length as i64).step_by(step) {
            ...
            if font_size > 0 {
                self.svg += &format!(
                    "<text x='{}' y='{}' font-family='Helvetica' font-size='{}'>{}M</text>",
                    x2, y2  as i32 - font_size, font_size, l/1000000
                    );
            }
    ...
    fn plot_chord(&mut self, dx: f64, dy: f64) {
        ...
        for sd in &self.result.sds {
            ...
            self.svg += &format!(r#"
                ...
                "#,
                pleft1.0, pleft1.1, self.radius, self.radius, pright1.0, pright1.1,
                pright2.0, pright2.1,
                self.radius, self.radius, pleft2.0, pleft2.1,
                pleft1.0, pleft1.1,
                color, color
                    );
        }
    ...
    fn title(&mut self, x: i32, y: i32, text: &str, font_size: i32) {
        self.svg += &format!(
            "<text x='{}' y='{}' font-family='Helvetica' font-size='{}'>{}</text>",
            x, y+font_size, font_size, text
            );
    }
}
