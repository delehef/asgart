...
impl Colorizer for PositionColorizer {
    ...
    fn color(&self, sd: &SD) -> String {
        let color =
            Srgb::from_hsv(self.gradient .get(sd.global_left_position as f64/self.total_length))
            .into_components();
...
impl FragmentColorizer {
    pub fn new(result: &RunResult) -> Self {
        let mut colors = (0..result.strand.map.len())
            .map(|i| rgb::Rgb::from_hsv(Hsv::new(
                30.0 + 330.0*(i as f64)/(result.strand.map.len() as f64), 1.0, 0.7
            )))
            .collect::<Vec<_>>();
        colors.shuffle(&mut thread_rng());


        let names = result.strand.map
            .iter()
...
impl Colorizer for FragmentColorizer {
    fn color_fragment(&self, name: &str) -> String {
        let color = self.colors
            .get(name).expect(&format!("Unable to get {}", name))
            .into_components();
        format!(
            "#{:02x}{:02x}{:02x}",
    ...
    fn color(&self, sd: &SD) -> String {
        let color = self.colors.get(&sd.chr_left).expect(&format!("Unable to get {}", &sd.chr_left)).into_components();
